...
SELECT 
    job_id ,
    CAST(job_work_from_home AS INT) AS job_work_from_home, -- From boolean to int
    CAST(job_posted_date AS DATE) AS job_posted_date, -- FROM Timestamp to date
    CAST(salary_year_avg AS DECIMAL(10,0)) AS salary_year_avg -- From double to decimal
FROM job_postings_fact
...
┌────────┬────────────────────┬─────────────────┬─────────────────┐
│ job_id │ job_work_from_home │ job_posted_date │ salary_year_avg │
│ int32  │       int32        │      date       │  decimal(10,0)  │
├────────┼────────────────────┼─────────────────┼─────────────────┤
│   4651 │                  0 │ 2023-01-01      │          110000 │
│   4699 │                  0 │ 2023-01-01      │           65000 │
│   4804 │                  1 │ 2023-01-01      │           90000 │
│   4810 │                  0 │ 2023-01-01      │           55000 │
│   4833 │                  0 │ 2023-01-01      │          120531 │
│   4846 │                  0 │ 2023-01-01      │          300000 │
│   5089 │                  0 │ 2023-01-01      │           51000 │
│   5123 │                  0 │ 2023-01-01      │          133500 │
│   5321 │                  0 │ 2023-01-01      │           77500 │
│   5325 │                  0 │ 2023-01-01      │          125000 │
└────────┴────────────────────┴─────────────────┴─────────────────┘
  10 rows                                               4 columns
...
SELECT 
    CAST(job_id AS VARCHAR) || '-' || CAST(company_id AS VARCHAR) , -- Concatinating 2 strings
    CAST(job_work_from_home AS INT) AS job_work_from_home, -- From boolean to int
    CAST(job_posted_date AS DATE) AS job_posted_date, -- FROM Timestamp to date
    CAST(salary_year_avg AS DECIMAL(10,0)) AS salary_year_avg -- From double to decimal
FROM job_postings_fact
...
────────────────────────────────────────────────────┬────────────────────┬─────────────────┬─────────────────┐
│ ((CAST(job_id AS VARCHAR) || '-') || CAST(company_ │ job_work_from_home │ job_posted_date │ salary_year_avg │
│                  id AS VARCHAR))                   │                    │                 │                 │
...
SELECT 
    job_id :: VARCHAR || '-' || company_id :: VARCHAR AS unique_id , -- Concatinating 2 strings
    job_work_from_home :: INT AS job_work_from_home, -- From boolean to int
    job_posted_date :: DATE AS job_posted_date, -- FROM Timestamp to date
    salary_year_avg :: DECIMAL(10,0) AS salary_year_avg -- From double to decimal
FROM job_postings_fact
WHERE  salary_year_avg IS NOT NULL
LIMIT 10;
-- using :: operator
...
┌───────────┬────────────────────┬─────────────────┬─────────────────┐
│ unique_id │ job_work_from_home │ job_posted_date │ salary_year_avg │
│  varchar  │       int32        │      date       │  decimal(10,0)  │
├───────────┼────────────────────┼─────────────────┼─────────────────┤
│ 4651-4651 │                  0 │ 2023-01-01      │          110000 │
│ 4699-4699 │                  0 │ 2023-01-01      │           65000 │
